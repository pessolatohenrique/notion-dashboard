...
describe("GET /database - show detail from database", () => {
  it("should show detail when exists", async () => {
    const result = await request(app).get(
      "/database/0c72c795-7b93-4de7-8e34-758490319795"
    );
  ...
  it("should show detail when exists with cache flag", async () => {
    const result = await request(app).get(
      "/database/0c72c795-7b93-4de7-8e34-758490319795?fromCache=true"
    );
    expect(result.statusCode).toEqual(200);
    expect(result.text).toContain("id");
  });
  ...
  it("should show no content when database not exists", async () => {
    const result = await request(app).get("/database/aaaaa");
    expect(result.statusCode).toEqual(204);
  });

  it("should show no content when database not exists with cache flag", async () => {
    const result = await request(app).get("/database/aaaaa?fromCache=true");
    expect(result.statusCode).toEqual(204);
  });
});
